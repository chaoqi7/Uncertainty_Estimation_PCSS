...
DATA_PATH = "/home/qc/Data/Stanford3dDataset_v1.2_Aligned_Version"
...
g_var2color = {cls: [255-2*cls,255-math.floor(1.583*cls),245-cls] for cls in range(120)}

g_Trueorwroncolor = {True:[0,255,0], False: [255,0,0]}
...
def collect_point_label_part(anno_path, out_filename, file_format='txt'):
    ...
    #part_labels = pointcloud_partition(np.array(data_label[:, 0:3], dtype='float32'),np.array(data_label[:, 3:6], dtype='uint8'), data_label[:, 6], out_filename)
    part_label = pointcloud_partition(data_label[:, 0:3].astype('float32'),
                                       data_label[:, 3:6].astype('uint8'), data_label[:, 6], out_filename)
        ...
        for i in range(data_label.shape[0]):
            fout.write('%f %f %f %d %d %d %d %d\n' % \
                          (data_label[i,0], data_label[i,1], data_label[i,2],
                           ...
                           data_label[i,6],part_label))
        fout.close()
    elif file_format=='numpy':
        data_label_final=  np.concatenate((data_label,part_label.reshape(part_label.shape[0],-1)),axis=1)
        np.save(out_filename, data_label_final)
    else:
