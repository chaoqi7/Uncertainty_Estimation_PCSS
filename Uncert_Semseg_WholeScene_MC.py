...
def parse_args():
    ...
    parser.add_argument('--times_var', type=int, default=10,
                        help='times for var computation [default: 10]')
    parser.add_argument('--save_output', type=bool, default=False, help='save_output or not [default: False]')
...
def main(args):
    ...
    file_handler = logging.FileHandler('%s/logs/Uncert_Semseg_WholeScene_MC.txt' % experiment_dir)
    ...
    with torch.no_grad():
        ...
        for batch_idx in range(num_batches):
            ...
            if args.visual:
                fout = open(os.path.join(visual_dir, scene_id[batch_idx] + '_pred_MC.obj'), 'w')
                fout_gt = open(os.path.join(visual_dir, scene_id[batch_idx] + '_gt_MC.obj'), 'w')
            if args.visual_var:
                fout_uncert = open(os.path.join(visual_dir, scene_id[batch_idx] + '_uncert_MC.obj'), 'w')
                fout_trueorwron = open(os.path.join(visual_dir, scene_id[batch_idx] + '_trueorwron_MC.obj'), 'w')
            ...
            for k in range(args.times_var):
                ...
                if k == 0:
                    output_mean = label_pool * 0 # for pred_label
                    output_square = label_pool * 0 # for unct_square
                    entropy_mean = label_pool.mean(1) * 0 # for unct_entropy
                output_mean += label_pool
                output_square += label_pool.pow(2)
                entropy_mean += acquisition_func('e', label_pool)
            ...
            #pred_label = pred_label.contiguous().cpu().data.numpy()
            ...
            if args.save_output:
                output_pred_unct(args, experiment_dir, 'MC_'+scene_id[batch_idx], labels, preds, uncts_r, uncts_e,
                                 uncts_b, uncts_v)
            timeend3 = time.time()
            ...
            for i in range(whole_scene_label.shape[0]):
                ...
                if args.visual_var:
                    ...
                    whole_scene_data[i, 0], whole_scene_data[i, 1], whole_scene_data[i, 2], color_var[0], color_var[1],
                    color_var[2]))
                    fout_trueorwron.write('v %f %f %f %d %d %d\n' % (whole_scene_data[i, 0], whole_scene_data[i, 1], whole_scene_data[i, 2], color_tw[0], color_tw[1],color_tw[2]))
            if args.visual:
                fout.close()
                fout_gt.close()
            if args.visual_var:
