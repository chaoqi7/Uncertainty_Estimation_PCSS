...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='pointnet2_sem_seg_NSA_MC', help='model name [default: pointnet_sem_seg]')
...
def main(args):
    ...
    shutil.copy('models/pointnet_util_Sampling.py', str(experiment_dir))
    ...
    for epoch in range(start_epoch,args.epoch):
        ...
        for i, data in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            loss = criterion(seg_pred, target, trans_feat, weights,NUM_CLASSES)
            loss.backward()
            optimizer.step()
        ...
        #if epoch % 5 == 0:
        if epoch % 5 == 0:
            logger.info('Save model_dp...')
            savepath = str(checkpoints_dir) + '/model_dp.pth'
        ...
        with torch.no_grad():
            ...
            if mIoU >= best_iou:
                best_iou = mIoU
                logger.info('Save best_model_dp...')
                savepath = str(checkpoints_dir) + '/best_model_dp.pth'
                log_string('Saving at %s' % savepath)
                state = {
                    'epoch': epoch,
                    'class_avg_iou': mIoU,
